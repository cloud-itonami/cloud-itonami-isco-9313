(ns construction.actor
  "ConstructionLabourActor — the ISCO-08 9313 community construction actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one construction operation request
  ...
  The unconditional invariant: the ConstructionLabourAdvisor can never
  directly commit a record the ConstructionLabourGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [construction.advisor :as advisor]
            [construction.governor :as governor]
            [construction.store :as store]))

(defn build-graph
  "Build a compiled ConstructionLabourActor graph. `store` implements
  `construction.store/Store`. `advisor` implements
  `construction.advisor/Advisor` (defaults to `mock-advisor`).
